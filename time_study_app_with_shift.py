...
for key, default in {
    ...
    "total_time": 0.0,
    "start_timers": False,
    "input_key_suffix": str(random.randint(1000, 9999)),  # forces rerender
}.items():
...
def show_add_elements_page():
    ...
    input_key = f"element_input_{st.session_state.input_key_suffix}"
    with st.form("add_element_form"):
        new_element = st.text_input("Element name", key=input_key)
        ...
        if submitted and new_element.strip():
            ...
            if name not in st.session_state.elements:
                ...
                st.session_state.input_key_suffix = str(random.randint(1000, 9999))
                st.rerun()
            else:
