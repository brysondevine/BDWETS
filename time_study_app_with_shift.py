...
def show_add_elements_page():
    ...
    with st.form("add_element_form"):
        new_element = st.text_input("Element name")
...
def show_timing_page():
    st_autorefresh(interval=100, limit=None, key="stopwatch")
    ...
    blinking_dot = "🔴" if int(time.time() * 10) % 2 == 0 else "⚫"
    total_placeholder.markdown(f"### {blinking_dot} Total Time: `{format_time(total)}`")
